...
buttonElement.addEventListener("click", function () {
    ...
    if (selectedValue === "completed") {
        ...
        const uncompleted = allItems.filter((item) => item.style.textDecoration !== "line-through");
        uncompleted.forEach((item) => (item.style.display = "none"));
    }
    else if (selectedValue === "uncompleted") {
        allItems.map((item) => (item.style.display = ""));
        const completed = allItems.filter((item) => item.style.textDecoration === "line-through");
